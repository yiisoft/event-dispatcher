...
If you're using a DI container, you can leverage `Yiisoft\EventDispatcher\Support\ListenerCollectionFactory` to
make creation of `ListenerCollection`s simpler and more straightforward. Factory uses
the `\Psr\Container\ContainerInterface` and [yiisoft/injector](https://github.com/yiisoft/injector) inside
to resolve event listeners in runtime. This means you can create simpler event listener configuration:

- A regular callable:
    ```php
    fn (EventName $event) => someStuff($event)
   ```
- A regular callable with additional dependencies that are obtained from container:
    ```php
...
- Static method call from a class:
    ...
    ```
  In this case event dispatcher will call `SomeClass::methodName($event)` with method dependencies obtained from container.
- Non-static method in a class:
    ```php
    [SomeClass::class, 'methodName']
    ```
    `SomeClass` object will be retrieved from a DI container with dependency resolving. Then the event dispatcher will
    call `$someClass->methodName($event)`. Dependencies will be obtained from container.
    
All the dependency resolving is done in a lazy way: dependencies will not be resolved before the corresponding
event will happen.
